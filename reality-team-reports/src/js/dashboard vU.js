...
const dom = {
    templates: {
        head: "<thead><th>VISITS</th><th>BOOKINGS</th><th>VALUE (CR.)</th><th>TARGET</th></thead>",
        statushead: "<thead><th>DATE</th><th>VISITS</th><th>BOOKINGS</th><th>VALUE (CR.)</th><th>TARGET</th></thead>",
        body: (id, data) => `<tr data-parent-id="${id}" data-id="${data["STAFF NAME"]}" class="hide"><td>${data["VISITS"]}</td><td>${data["BOOKINGS"]}</td><td>${data["VALUE (CR.)"]}</td><td></td></tr>`,
        ...
        status: (btns, data) => `<tr data-id="${data["ID"]}" data-date="${data.YEAR}/${data.MONTH}/${data.DATE}" class=""><td class="date" data-sort="${data.YEAR}${data.MONTH}${data.DATE}">${data.DATE}/${data.MONTH}/${data.YEAR}</td><td class="visits">${data["VISITS"]}</td><td class="bookings">${data["BOOKINGS"]}</td><td class="value">${data["VALUE (CR.)"]}</td><td></td><td class="btns-container">${btns}</td></tr>`,
    },
    add: {
        ...
        toggle: {
            view: function () {
                ...
                if (currentViewNo === 0) {
                    ...
                    _$("#view-switcher").innerText = "FORM"
                    _$("#info mark").innerText = "Rejected Requests"
                }
                else {
                    currentViewNo = 0
                    _$("#form").style.display = "flex"
                    _$("#view-switcher").innerText = "PENDING"
                    _$("#info mark").innerText = "FORM"
                    return
                }
            },
